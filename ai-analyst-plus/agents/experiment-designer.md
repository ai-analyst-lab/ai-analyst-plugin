...
Compute from {{DATASET}} using the `experiment_stats` library bundled in the experiment skill (`scripts/experiment_stats/`):

```python
from experiment_stats import power_proportion, power_mean, duration_estimate
...
- If 2-4 weeks → **VIABLE (long)**: proceed but flag that results will take time
- If 4-8 weeks → **MARGINAL**: consider increasing MDE, using a more sensitive metric, or a different design
- If > 8 weeks → **NOT_VIABLE** as a standard A/B: recommend quasi-experimental approach (Step 3) or a decision without experimentation
...
## Skills Used
- the `metric-spec` skill — for defining primary, secondary, and guardrail metrics with full specifications
- the `guardrails` skill — for selecting guardrail metrics that pair with the primary success metric
- the `triangulation` skill — for sanity-checking baseline metrics and power estimation inputs

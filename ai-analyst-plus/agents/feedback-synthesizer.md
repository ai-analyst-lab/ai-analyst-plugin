...
ESTIMATED EFFORT:
  [total time for all layers — this block is the one place in the analysis-design
   flow where a time estimate is allowed; the design brief and prioritized steps
   stay estimate-free]
...
- **`/analysis-design` skill** (final stage of the /analysis-design flow)
- **User** (directly actionable V2 plan)
- **The analyst session itself** (executes the V2 plan per the analyst-core method)

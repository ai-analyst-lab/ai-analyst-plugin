...
description: >-
  Turn a vague hunch or stakeholder request into a rigorous analysis plan via a staged pipeline
  (hypothesis sharpening, confound scan, V1, feedback, V2 redesign). Trigger on "I think X caused
  Y", "design an analysis for", "what's driving", "V2 after feedback", "stakeholder comments",
  "redesign the analysis". Vs question-framing (the lightweight framing gate before every
  analysis): use THIS only for a real hunch, confound scan, or V1 to V2 redesign.
---
...
/analysis-design "{{HUNCH}}"
  ...
  │  (Skill runs the analysis per the   │
  │   analyst-core method, with direct  │
  │   Python/SQL as appropriate)        │
  │                                     │
...
2. **Prioritized Steps** — ordered by information value (what eliminates the most uncertainty fastest). Use a **3-column table** (no time estimates; the ONE place a time estimate is allowed in this flow is the feedback-synthesizer agent's V2 plan, which has an ESTIMATED EFFORT block):
   | # | Step | Why First |
   |---|------|-----------|
...
Generate the document via the Google Doc Export skill (requires the Google Drive connector), with its formatting conventions (navy blue headings, bold callout labels, proper table spacing). If the connector is not available, output as clean markdown.
...
- **Question Framing skill**: for simple questions its Question Brief (the Ladder plus the 7-field Analysis Design Spec) is sufficient; this skill supersedes it for complex investigations.
- **`/stress-test`** — can be invoked independently on ANY analysis plan (not just ones produced by this skill)
- **analyst-core skill** — governs Stage 3b V1 execution (profile, compare, trace, checks)

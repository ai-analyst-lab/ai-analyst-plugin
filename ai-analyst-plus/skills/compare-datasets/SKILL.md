...
description: >-
  Compare metrics, findings, and patterns across two or more connected datasets. Trigger on
  "compare datasets", "is this pattern the same across", "how does X differ between datasets",
  "cross-dataset comparison", "compare metrics between", or asking whether a finding is unique to
  one dataset. Also offer proactively after analyzing dataset A when the user switches to dataset
  B for similar queries.
---
...
### Export reconciliation (run FIRST when the two datasets are versions or exports of the same table)

Before any pattern comparison, reconcile the exports mechanically, in Python:
1. Row counts: report both counts and the difference (absolute and percent).
2. Schema diff: columns present in one and not the other; for matching names, dtype differences.
3. Key reconciliation: if a plausible key column exists (unique in at least one side), report keys
   present in only one side, with a small sample of the missing rows.
4. Shared-column value diff on the intersection of keys: count rows where values differ, per column.
Report these four before saying anything about patterns. "Are these the same?" is answered by this
step, not by eyeballing distributions.

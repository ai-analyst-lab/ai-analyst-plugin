...
name: data-inspect
description: >-
  Show the active dataset's schema: tables, columns, row counts, relationships. Trigger on "/data"
  or "/data {table}", "what tables do I have?", "show me the schema", "what columns are in the
  users table?", "list tables", "describe the data", "data dictionary". DISAMBIGUATION: schema
  inspector only; for "tell me about this data" use data-map; for browsing use /explore; for deep
  statistical profiling use data-profiling.
---
...
4. **Never query actual data** — this skill shows structure only (schema, relationships). For data exploration, use the `/explore` skill.

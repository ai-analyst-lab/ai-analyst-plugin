...
name: data-map
description: >-
  Produce a cross-table data health map for the active dataset: row counts, date alignment, join
  rates, relationships, opening hypothesis. Trigger on dataset-wide open questions: "tell me about
  this data", "what's in here", "give me an overview", "map out the data", "what do I have", "what
  does this data look like". NOT for table-scoped questions (data-quality-check), /explore
  follow-ups, schema listing (/data), or deep profiles (data-profiling).
---
...
1. Read `.knowledge/active.yaml` to get `active_dataset`. If missing, halt and tell the user to run `/connect-data` (knowledge-bootstrap can create the rest of the tree).
...
5. Verify connectivity through the session's active data connection (a connected warehouse, local DuckDB, or the CSV files, in that order of preference). Announce which source is active in one line.
...
After running, append each query to `.knowledge/query-log.md` in the working folder: a dated line with the purpose, the SQL, and the row count.
...
- **Connection failures:** Fall back to the next available source (warehouse → local DuckDB → CSV) and note which source is active.

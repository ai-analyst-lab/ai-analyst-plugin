...
name: deck-critique
description: >-
  Score a presentation slide-by-slide against the Data Story Checklist (SO-WHAT, STAKES, EVIDENCE,
  ASK): per-slide scorecards, anti-pattern flags, an A-F grade, and prioritized fixes. Trigger on
  "review my deck", "critique this presentation", "score my slides", "evaluate this deck", "deck
  feedback", "slide review", "check my slides", or a Marp .marp.md path or Google Slides URL.
  Auto-fire before /deck-rescue; the critique is its prerequisite input.
---
...
**For Google Slides:**
Fetch each slide's content through the Google Slides connector (or Google Drive connector export) and extract the same fields as above: title, body text, bullets, whether the slide has images or tables, and word count. If no connector is available, ask the user to export the deck to Marp markdown or plain text and critique that.
...
## Skills Used
- Deck parsing (Step 1) is done inline by this skill
- `/stakeholder-communication` — for evaluating audience-appropriateness of stakes and asks

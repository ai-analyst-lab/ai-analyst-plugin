...
name: deck-rescue
description: >-
  Take a failing deck end-to-end and produce a complete rewrite with proper narrative structure
  (Context-Tension-Resolution arc, action headlines, a specific ask). Use when a deck scores D or
  F on /deck-critique, or the user says "rescue", "rewrite", "redo", or "fix" an entire deck,
  "this deck is a mess", "completely rebuild these slides", "salvage this presentation". For minor
  fixes to a few slides use /slide-transform instead.
---
...
- **`{{THEME}}`** (optional): theme to apply — defaults to `analytics` per the auto-selection table in `reference/presentation-standards.md` (Marp-format analyst deliverables default light); pass `analytics-dark` for workshop/talk contexts
...
Parse every slide yourself. For a Marp file: strip the YAML frontmatter, split on `---` slide separators, and for each slide record the title (first heading), the body text, the bullets (lines starting with `-` or `*`), whether it contains a chart (image reference) or a data table, and the word count. For Google Slides: fetch each slide's text through the Google Slides connector and record the same fields. Then extract ALL data, findings, and claims from the original deck — separate signal from noise:
...
**If not:** Do the story extraction yourself on the content inventory, in five moves:
1. Catalog every discrete finding, claim, or data point as a numbered flat list (numbers, comparisons, trends, anomalies, claims, recommendations). Don't interpret yet.
2. Score each finding on four 1-5 dimensions (Surprise, Business Impact, Audience Relevance, Evidence Strength) and sum to a composite out of 20. If no audience was given, score relevance for a general executive audience.
3. Pick Story #1 (highest composite), then a non-redundant Story #2 and #3. Check that Story #1 has a twist: the gap between what the audience expects and what the data shows. A story without surprise is just a report.
4. Write the lead-story brief: HEADLINE (one sentence), THE TWIST, THE STAKES (quantified), THE EVIDENCE (2-3 specific data points), THE ASK.
5. List what to CUT (off-story, redundant, too granular, unsurprising) with a one-line rationale each. An empty cut list means no editorial judgment was applied.

Save the extraction to `working/story_extraction_{{DATE}}.md`. This identifies the 1-3 stories worth telling and the #1 story to lead with.
...
**Marp formatting:**
- Use the `{{THEME}}` theme (default: `analytics`); the theme is a styling spec in `reference/presentation-standards.md`, applied through the deck's frontmatter and inline styles (no CSS file ships with this plugin)
- Apply appropriate slide classes from the theme spec
...
4. Deliver the deck as the `.marp.md` file (it is readable markdown as-is). If the user wants a rendered file and Marp CLI is available: `npx @marp-team/marp-cli working/deck_rescue_{{DATE}}.marp.md --pdf` (or `--html`), using Marp's built-in styling; add `--theme <path>` only if the user supplies their own Marp CSS file. No CSS theme files ship with this plugin.
```
...
- **Render:** Use Marp CLI to produce the PDF or HTML when the user wants a rendered file; the `.marp.md` itself is the core deliverable
...
## Skills Used
- `/deck-critique` — for initial diagnosis (Step 1)
- `reference/presentation-standards.md (in this skill)` — for Marp theme application
- Story extraction (Step 3) and deck parsing (Step 2) are done inline by this skill

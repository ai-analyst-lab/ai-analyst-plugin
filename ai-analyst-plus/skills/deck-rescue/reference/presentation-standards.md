# Presentation standards (reference; formerly the presentation-themes skill)
...
- Styling: apply these values through Marp frontmatter and inline styles (no CSS theme file ships with this plugin; use `--theme <path>` only with a user-supplied CSS)
...
**CSS scoping warning (applies only when the user supplies their own analytics-dark CSS file):** When extending that CSS with new component styles, ensure overrides cover all three dark-specific variants if they have unique backgrounds:
```css
...
This uses Marp's built-in styling plus the deck's frontmatter and inline styles. No CSS
theme files ship with this plugin; add `--theme <path-to-css>` only when the user
supplies their own Marp CSS file.

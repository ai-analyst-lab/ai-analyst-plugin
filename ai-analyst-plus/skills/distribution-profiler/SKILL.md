...
For connector-attached warehouses, query through the connector. For local DuckDB
or CSV sources, connect directly in Python (a read-only DuckDB connection, or
pandas over the CSV files).
...
Use the diagnostic results and the Identification Flowchart in this skill's
bundled reference, `reference/statistical-distributions.md` (in this skill's
directory, next to this SKILL.md), to identify the most likely distribution.

Read that distribution's card in the reference for the details on valid summary
stats, tests, transformations, and A/B implications.
...
The distribution reference ships with this skill at
`reference/statistical-distributions.md` (next to this SKILL.md).

It covers 12 distributions plus zero-inflated models, with detection
heuristics, valid summary statistics, recommended tests, transformations,
and A/B testing implications for each.

**When to read it:** After identifying the distribution in Step 3, read the
specific card for that distribution to get detailed guidance. Don't load
the entire file if you only need one card.

Sections:
- Identification Flowchart: the decision tree Step 3 uses
- Distribution cards: one card per distribution (detect, valid stats, tests, transform, A/B), including zero-inflated
- Practical guidance: t-test robustness rules, family tree, effect sizes, censoring
...
5. Use the bundled reference (`reference/statistical-distributions.md`) for detailed recommendations, not your own memory

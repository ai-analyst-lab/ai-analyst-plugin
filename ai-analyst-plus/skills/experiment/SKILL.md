...
**Agent:** the `experiment-analyzer` plugin agent
**Flow:**
...
3. Treatment effect analysis using coded helpers:
   ```python
   from experiment_stats import welch_test, proportion_test, ratio_metric_test
...
**Agent:** the `experiment-interpreter` plugin agent
**Flow:**
...
**Agent:** the `experiment-readout` plugin agent
**Flow:**
...
**Agent:** the `experiment-monitor` plugin agent
**Flow:**
...
All statistical work uses the coded library bundled at `scripts/experiment_stats/`:

...
name: guardrails
description: >-
  Pair every success metric with guardrail metrics and check for trade-offs before presenting
  improvements as wins. Trigger when defining a metric or spec, and on any positive metric
  movement: "X improved", "X is up", "X increased", "better performance on X", "we're seeing gains
  in X". Never present a metric improvement without the guardrail check; an improvement with a
  degraded guardrail is a trade-off, not a win.
---

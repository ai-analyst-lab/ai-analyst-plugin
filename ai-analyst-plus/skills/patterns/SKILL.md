...
name: patterns
description: >-
  Browse, search, and leverage recurring analytical patterns discovered across past analyses.
  Trigger on "what patterns have we seen?", "have we seen this behavior before?", "show me
  recurring patterns", "what's consistent in our data?", "any known patterns about
  [metric/segment]?", "search patterns for [keyword]". Also updates pattern entries after each
  analysis archive and can remind at session start.
---
...
After an analysis is saved to `.knowledge/analyses/` (any skill that writes a run
record there), scan the new analysis for potential patterns:

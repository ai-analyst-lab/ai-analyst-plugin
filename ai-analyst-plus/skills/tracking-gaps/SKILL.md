...
name: tracking-gaps
description: >-
  Assess whether the data needed for an analysis exists, identify what's missing, and produce
  prioritized instrumentation requests for engineering. Trigger on "do we track...", "is there
  data for...", "can we measure...", writing an instrumentation request, queries returning
  unexpected nulls, or verifying data exists before committing to an analytical approach. Produces
  workarounds for partial data and realistic expectations.
---
...
## When to Use
Apply this skill after the `/explore` skill inventories available data, when an analysis requires data that might not exist, or when initial query results suggest incomplete tracking. Run before committing to an analysis approach.
...
2. **Switch datasets:** Use `/datasets` and `/switch-dataset` (both in the datasets skill) to move to a product analytics database that tracks feature usage and user roles.
